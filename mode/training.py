...
def train(cfg: DictConfig) -> None:
    ...
    log_config_and_system_info(cfg)
...
def load_or_create_model(cfg):
    chk = get_last_checkpoint(Path.cwd())
    if chk is not None:
        return getattr(models_m, cfg.model["_target_"].split(".")[-1]).load_from_checkpoint(chk.as_posix())
    else:
...
def log_config_and_system_info(cfg):
    log_rank_0(f"Training with the following config:\n{OmegaConf.to_yaml(cfg)}")
    log_rank_0(f"Repo commit hash: {get_git_commit_hash(Path(hydra.utils.to_absolute_path(__file__)))}")
    log_rank_0(print_system_env_info())
